...
public class ControlHandleFoodDetail {
    ...
    public static final String TABLE_ALIM = "tb_alim";
    ...
    public void initialize() {
        ...
        carboFood.focusedProperty().addListener((obs, oldVal, inFocus) -> {
            verifyAndCalcucaleTotalKcal(inFocus,carboFood);
        });

        protFood.focusedProperty().addListener((obs, oldVal, inFocus) -> {
            verifyAndCalcucaleTotalKcal(inFocus,protFood);

        });

        fatFood.focusedProperty().addListener((obs, oldVal, inFocus) -> {
            verifyAndCalcucaleTotalKcal(inFocus,fatFood);

        });

        fiberFood.focusedProperty().addListener((obs, oldVal, inFocus) -> {
            verifyAndCalcucaleTotalKcal(inFocus,fiberFood);
    ...
    private void savingData() {
        if (isNewFood == true) {
            try (Connection conn = DriverManager.getConnection(CONNECTION_STRING);
                ...
                String sql = "INSERT INTO " + TABLE_ALIM + " (ALIMENTO,PRO,CAR,GRA,FIB,KC) VALUES ('" + nameFood.getText() + "',"
            ...
            try (Connection conn = DriverManager.getConnection(CONNECTION_STRING);
                ...
                query = conn.prepareStatement(
                 "UPDATE " + TABLE_ALIM + " SET ALIMENTO = ?, PRO = ? , CAR = ?, GRA = ?, FIB = ?, KC = ? "
    ...
    static public  void deleteData() {
        ...
        try(Connection conn = DriverManager.getConnection(CONNECTION_STRING);
            ...
            ResultSet results = statement.executeQuery(sqlSelectFood);


            if(!results.next()) {
                Statement statementeDelete = conn.createStatement();
                String sqlDeleteFood = "DELETE FROM " + TABLE_ALIM + "  WHERE " + COLUMN_ID_ALIM + " = " + rowModifyRecord;
                statementeDelete.execute(sqlDeleteFood);
                statementeDelete.close();
            }else{
                Alert a = new Alert(Alert.AlertType.NONE);
                a.setAlertType(Alert.AlertType.ERROR);
    ...
    private void loadValues(){
        ...
        try(Connection conn = DriverManager.getConnection(CONNECTION_STRING)) {
            ...
            Statement statement = conn.createStatement();
            ResultSet results = statement.executeQuery("SELECT * from " + TABLE_ALIM +" where " +  COLUMN_ID_ALIM +" = "+ rowModifyRecord );
            if(results.next()){
